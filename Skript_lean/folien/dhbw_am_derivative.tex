...
\begin{frame}{Ableitungsdefinitionen}
...
\begin{frame}{Definition der Ableitung entlang eines Filters }
    \begin{block}{Ableitung in mehreren Dimensionen}
    Eine Funktion \( f: \mathbb{R}^n \to  \mathbb{R}^m\) hat die (lineare) Abbildung \( df \) als Ableitung, falls
    \begin{align}
    \label{diff}
    f(a + h) = f(a) + df(a)h + o(h),
\end{align}
    Die Ableitung wird auch mit $f':=df$ bezeichnet.
