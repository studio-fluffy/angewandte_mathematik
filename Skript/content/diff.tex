...
Für eine differenzierbare Funktion  $f: U \to \mathbb{R}$ gilt für $a \in U$:
\begin{align*}
|| \nabla f(a) || = \max \{ \partial_h f(a) \; | \; ||h|| = 1 \} \; .
\end{align*} 
\end{Satz}
\begin{proof}
...
\item $f (f \cdot g) = g(a) d(f) + f(a) dg$
\item $d(f + g) = df + dg$
\end{itemize}
\end{Bemerkung}
\begin{proof}
...
\end{align*} 
mit $a$ verbinden. Damit ist $f(a + h) - f(a) = \sum_{i=1}^{n} \bigl( f (a_i)   - f(a_{i-1})   \bigr)$ und  mit $\varphi_i(t) : = f(a_i + t e_i)$ gilt 
...
\begin{align*}
\frac{d(f \circ \gamma)}{dt} = df(\gamma(t))\gamma'(t) = \sum_{i=1}^n  \frac{\partial f (\gamma(t))}{\partial x_i} \gamma'_i(t)
\end{align*} 
\end{Satz}
...
Da nach Konstruktion $F(0) = a$ und $F(1)= f(x)$ folgt insgesamt die Behauptung.
\end{proof}
...
\subsection{Lokale Maxima und Minima}
...
\begin{align*}
\frac{\partial}{\partial x_{1}}  = \cdots  = \frac{\partial}{\partial x_{n}}  = 0 \;.
\end{align*}
...
Da $F_k$ eine Funktion einer Veränderlichen ist, git $F'(0) = 0$. Da $\frac{\partial}{\partial x_k} f(a) = F_k'(0)$ folgt die Behauptung.
\end{proof}
