...
\fbox{\parbox{\linewidth}{
$\star \star \bigstar $  Das Differential einer differenzierbare Funktion  kann als eine Abbildung von Tangenten interpretiert werden.
}}
...
\State Initialisiere Genauigkeit $\epsilon > 0$.
	...
	\State Berechne $d_k :=  - \nabla f(x_k)$ und wähle $\alpha_k$ klein genug (siehe oben).
...
Wende Gradientenverfahren an (vergleiche \ref{gradientenabstieg}):
...
\State Initialisiere Genauigkeit $\epsilon > 0$.
	...
	\State Berechne $d_k :=  - \nabla L_D(\omega_k)$ und wähle $\alpha_k$ klein genug.
...
\State Wähle Teilmenge $D_0' \subset D$
	...
	\State Berechne $d_k :=  - \nabla  L_{D'_k}(\omega_k)$ und wähle $\alpha_k$ klein genug.
        \State Setze $\omega_{k+1} := \omega_k  + \alpha_k d_k$. \Comment{Absteigen}
