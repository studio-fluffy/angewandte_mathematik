...
$(\varphi + \psi)(x) := \sum_{k=1}^m \sum_{j=1}^l   (c_k + u_j) 1_{I_{k,j}}$ mit $I_{k,j}:= I_k \cap I_j$ eine Treppenfunktion (nach entsprechender Umnummerierung zu einem einzigen Summenzeichen \href{https://de.wikipedia.org/wiki/Cantors_erstes_Diagonalargument}{wiki}).
\end{block}
...
\begin{frame}
    \frametitle{Angewandte Mathematik}
\framesubtitle{Beweis}
Wähle zu $\epsilon > 0$ Hüllreihe $\phi_k := \sum_i c_{ik}1_{I_{ik}}$  von $f_k$ mit Inhalt 
$I(\phi_k) \leq || f_k||_1 + \frac{\epsilon}{2^k}$. Mit der geometrischen Reihe und $\epsilon \to 0$ folgt die Behauptung.  
 \end{frame}
...
\begin{frame}
    ...
    \begin{block}{Integral eines Quaders}
Für einen abgeschlossenen Quader A gilt
$$ ||1_{A} ||_1 = \mu(A) = \int 1_{A}$$
\end{block}
